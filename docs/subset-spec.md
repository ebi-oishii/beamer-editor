...
- `decktext` は現行の語彙を維持する。可: インライン要素(§2.5)、数式(§2.6)、改行 `\\`、`itemize` / `enumerate`(本文と同じくネスト 3 段まで)
...
- TikZ と生 LaTeX は通常フローで扱う。#81 の部分コンパイルは PR #112 で実装中であり、merge まではプレビューをプレースホルダとする。キャンバスへ置く必要があれば PDF / PNG に事前変換して `deckimage` を使う。表の GUI 編集は #12 で扱う
