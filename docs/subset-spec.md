...
- 通常フレームの段落・箸条書き・画像は、GUI の「自由配置にする」で `deckcanvas` の `decktext` / `deckimage` へ移せる(位置は移動時の表示位置から取り、残ったフロー要素は詰まる)。表示条件が変わるもの(overlay 付きの block / `\item` の中、`\pause` との前後関係が移動先と異なるもの)は対象にしない。項目の唯一の内容を移すときは空になる `\item`(項目が 1 つならリスト)も取り除き、`center` の中の要素は箱の中で `\centering` を保つ。候補にできない要素を右クリックしたときは理由を表示する
