...
| 段階 | 形 |
|---|---|
| M2 時点 | エディタの隣(VS Code シェルなら同一ウィンドウ内)で Claude Code を開き、アドレスコピー(§3)で対象を貼って依頼する |
| Phase 9 | プレビュー・スライド一覧・ソースの選択から「AI に依頼」→ 依頼ボックス → エディタが `[adjust]` を合成し、デッキごとの永続セッション(claude CLI の resume / Agent SDK)へ送信 → ファイル監視で反映 |

どちらの形でも AI の編集はファイル経由で届き、エディタは**外部編集を undo 履歴の 1 ステップとして取り込む**。気に入らない微調整は、チャットで「戻して」と言っても、エディタで Cmd+Z しても戻せる(二重の安全弁)。
