...
そこで、人間と AI が同じテキストを読み書きできる「共通言語」を軸に据える。チームは LaTeX を書き慣れているため、共通言語は Beamer のサブセットとする。Markdown 系 DSL を正規の保存形式や TeX との混在形式には採らない。ただし、明示的な操作で Markdown 風の入力を直ちに正規形 Beamer へ変換する入力補助は許可する。
...
3. **二層モデルと段階的劣化。** ソースは「構造化サブセット」と「生 LaTeX ブロック」の二層に分類される。サブセット内は完全理解(HTML 描画・GUI 編集可)、サブセット外は部分コンパイル画像で確認する不透明な塊(内容編集はソースで行う)。サブセット外を書いてもエラーにはならず、編集体験が劣化するだけ。キャンバスへ自由配置したい場合は、PDF / PNG に事前変換して `deckimage` として置く。
...
選択クリックではソースへ書き込まない。実際にドラッグしたときだけ、幅と gesture 開始時の実測高さが本文より小さければ本文領域内へ収めて反映する。高さが不明または本文以上の要素は anchor のみ `0..1` に収め、収納不能な溢れは TeX の実測検証で判定する。
...
| 決定 | 却下した代替案 | 理由 |
|---|---|---|
| ソースは Beamer サブセット | Markdown DSL の正規保存形式・TeX との混在形式(Pandoc/Marp/Quarto)、Typst | チームが LaTeX を書き慣れており可読性の前提が成立。ソース = 出力言語なので変換忠実性の問題が消える。AI も Beamer をゼロショットで書ける。Markdown 風入力は、明示操作で即座に正規形 Beamer へ変換する入力補助としてのみ許可する |
...
`decktext` の語彙拡張を再検討するのは、通常フローまたは画像化では具体的な要件を満たせず、要素単位の round-trip・fallback・コメント保持・L014・undo の契約を成立させられる見通しが立った場合に限る。
...
| 日本語(CJK)対応 | tectonic(XeTeX)で日本語を組むにはフォント設定(fontspec + CJK フォント等)が必要 | **解消済(S2、2026-07-19)**。`packages/core/resources/deck-style-preamble.tex` の xeCJK + `\deckfont` で CJK フォントを設定し、`deck fonts fetch`(`packages/cli`)が Noto Sans CJK JP を取得・キャッシュ・配置する。バイナリは同梱しない。`fixtures/japanese.slide.tex` を tectonic で警告 0 コンパイル確認([theme-design.md](theme-design.md) §4) |
