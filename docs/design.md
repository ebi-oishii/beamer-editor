...
6. **AI はファイル経由で接続する。** v1 では AI 機能を内蔵しない。ソースファイルと CLI がインターフェースであり、任意のエージェント(Claude Code 等)がファイルを直接編集し、エディタはファイル監視で即時反映する。エージェントの作業手順・アドレッシング・SKILL.md による配布は [ai-protocol.md](ai-protocol.md) に定める。エージェントの検証手段は HTML プレビューではなく常に TeX(`deck check` / `deck snapshot`)であり、プレビューが検出できない溢れ・組版エラーをエージェントが先回りして潰す。
...
- 数式は KaTeX(同期・ミリ秒描画)。
- Beamer テーマの CSS 再現は v1 では 1 テーマのみ(候補: default または metropolis)。
...
最小セットに絞る。すべて「AST 変換 → 触れたフレームを正規形で再出力」で実装し、undo/redo はソースの編集履歴一本に統合する。

- スライド一覧(サムネイル): 並べ替え・複製・削除・挿入
- フレームタイトル・箇条書きのインラインテキスト編集
- 画像の差し替え(ファイル選択 → `\includegraphics` 書き換え)
...
これらの UI(プレビュー・スライド一覧・GUI 操作・依頼ボックス)は共有パッケージ `ui` に一度だけ実装し、VS Code 拡張と Electron の両シェルがそれをホストする(§7 の ShellHost 契約)。
...
| AI は非内蔵、ファイル + CLI がインターフェース | チャット UI 内蔵 | ファイル監視で任意のエージェントと接続でき、実装が薄い。内蔵チャットは後からいつでも追加可能 |
| シェルは VS Code 拡張 + Electron の両立て(同時並行) | どちらか単独、Tauri | 拡張はエディタ・undo・競合処理・診断表示を標準機能で得られ Claude Code と同一ウィンドウに同居できる。Electron は独立アプリとしての体験を検証できる。両方の体験を最初から確かめるため同時に持ち、二重化コストは UI の共有パッケージ化とシェルの薄いアダプタ化で抑える。Tauri は JS 可読性の点で不採用 |

## 6. 技術スタック

| 領域 | 採用 | 選定理由 |
|---|---|---|
| シェル (1) | VS Code 拡張 | エディタ・undo・競合処理・git diff・診断表示(Diagnostics)を標準機能で獲得。Claude Code と同一ウィンドウに同居し、ドッグフーディング到達が最短 |
| シェル (2) | Electron + electron-vite | 独立アプリとしての体験。チームの JS 可読性。ネイティブ依存は tectonic のみ |
| UI | React(共有パッケージ `ui`) | プレビュー・スライド一覧・GUI 操作・依頼ボックスを 1 回だけ実装し、両シェルの Webview / BrowserWindow で共用する |
...
└── apps/
    ...
    ├── desktop/           # Electron シェル(CodeMirror、fs、IPC)
    └── web/               # 同一 UI の Web ビルド(劣化モード)
```

制約:

- `core` と `renderer` は Node 固有 API・Electron API・VS Code API に依存しない(web でそのまま動く)。`compiler` のみ Node 依存。
- `ui` はシェルの機能(編集の適用、変更の購読、ファイルダイアログ、コンパイル呼び出し、AI 依頼の送信)を **ShellHost インターフェース**越しにのみ使う。シェル差はこの契約の実装(アダプタ)に閉じ込め、`ui` 本体には一切持ち込まない。両立て戦略のコストを抑える要がこの境界。

## 8. Web 版の扱い

コアとプレビューはブラウザでそのまま動く。Web 版で欠けるのは部分コンパイルと PDF 書き出しのみ。

- 第一段階: 生ブロックはプレースホルダ表示に劣化、書き出し不可、として提供する。
...
| 同時編集の競合 | エディタ内未保存変更中に外部(AI)がファイルを書き換えた場合 | 未編集バッファなら黙って再読込。編集中は通知して選択させる(v1 は単純な方式でよい) |
...
| シェル二重化のコスト | VS Code 拡張と Electron の同時維持で、UI グルー・テスト・ドッグフーディングの注意が分散する | UI は `ui` パッケージに集約しシェルは ShellHost アダプタのみに限定。パリティは fixture デッキの共通チェックリストで確認。負担が実装を圧迫する場合は片方を後回しに降格する判断を許容する |
