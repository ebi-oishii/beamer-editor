...
GUI で扱う自由配置は `deckcanvas` の層に集め、通常の Beamer フロー配置の上に重ねる。フレーム本文に `deckcanvas` がある場合、そのフレームはキャンバスモードとする。

- 通常フレーム: 既存の `columns`、`itemize`、`block` などで構成する
- キャンバスフレーム: `deckcanvas` の直下に位置指定オブジェクトを並べる。フロー要素は同じフレームに残せる(`deckcanvas` は 1 つまで)
- フレームタイトルは v1 では固定領域であり、GUI で移動しない
- `deckcanvas` の直下には絶対配置要素だけを置く
- フロー要素(段落・箸条書き・画像)は GUI の「自由配置にする」で `deckcanvas` へ移せる
