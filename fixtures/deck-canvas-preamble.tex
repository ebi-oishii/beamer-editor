...
% fixtures/measure-body-area.slide.tex の実測(2026-07-10、tectonic 0.16.9)による:
...
%   書式: DECKGEOM frame=<フレーム番号> page=<物理ページ番号> kind=<text|image> x=<pt> y=<pt> w=<pt> h=<pt>
...
\newcounter{deck@geometry}
\newcommand\deck@geomlog[1]{%
  % savepos is resolved from the previous pass.  Do not emit a guessed zero
  % coordinate on the first pass: the compiler will either make a final pass
  % with valid geometry or leave an explicit unresolved sentinel.
  \stepcounter{deck@geometry}%
  \edef\deck@geometry@label{deckgeom-\insertframenumber-\the\value{deck@geometry}}%
  \zsavepos{\deck@geometry@label}%
  \zref@ifrefundefined{\deck@geometry@label}{%
    \typeout{DECKGEOMERROR reason=unresolved}%
  }{%
    \dimen@=\zposx{\deck@geometry@label}sp\relax
    \@tempdima=\paperheight\relax
    \advance\@tempdima by -\zposy{\deck@geometry@label}sp\relax
    \typeout{DECKGEOM frame=\insertframenumber\space page=\the\c@page\space kind=#1 x=\the\dimen@\space y=\the\@tempdima\space w=\the\wd\deck@contentbox\space h=\the\dimexpr\ht\deck@contentbox+\dp\deck@contentbox\relax}%
  }}
