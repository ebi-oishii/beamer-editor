...
- 実装済み: パーサ + AST(Phase 1)、キャンバス正規形フォーマッタ + リンター基盤 L009/L011/L017/L018/L020(Phase 2 の一部)、マクロ展開器(Phase 3)、HTML プレビュー + KaTeX(Phase 4)、スタイル語彙 v1(S1)、Noto Sans CJK 対応(S2)、VS Code 拡張スキャフォールド(VS-1)+ 共有プレビュー UI `packages/ui`(VS-2)+ 編集追従プレビュー(VS-3)
- 次: VS-4(プレビュー → ソースジャンプ)以降([vscode-migration-plan.md](docs/vscode-migration-plan.md))、Phase 2 の残り(正規形の全域化・残りリント規則)
